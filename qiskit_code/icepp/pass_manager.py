...
class pass_manager():
    ...
    def auto_manager(self):
        ...
        if self.level == 2:  
            optimized_qc = self.aqcel_apply()[0]
            ...
            depth_ls = self.aqcel_apply()[1]
            depth_ls_tof = self.aqcel_apply()[2]
            depth_cnot = self.aqcel_apply()[3]
            
            return [optimized_qc, transpiled_qc, depth_ls, depth_ls_tof, depth_cnot]
    ...
    def aqcel_apply(self):
        ...
        if self.measure_type == 'cc':
            qc3 = compiler.remove_controlled_operations(qc2).run_cc()[0]
            depth_ls = compiler.remove_controlled_operations(qc2).run_cc()[1]
            depth_ls_tof = compiler.remove_controlled_operations(qc2).run_cc()[2]
            depth_cnot = compiler.remove_controlled_operations(qc2).run_cc()[3]
        ...
        return [qc5, depth_ls, depth_ls_tof, depth_cnot]
